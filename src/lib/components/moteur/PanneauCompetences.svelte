<script lang="ts">
	...
	import { basculerCompetence, getCompetences } from '$lib/apis/hermes';
	...
	const load = async () => {
		...
		erreur = '';
		try {
			const res = await getCompetences(localStorage.token);
			const source = res?.skills ?? res ?? [];
			skills = Array.isArray(source)
				? source.map((skill: any) => ({
						name: `${skill.name ?? skill.id ?? ''}`,
						category: skill.category,
						description: skill.description,
						enabled: Boolean(skill.enabled ?? skill.active)
					}))
				: Object.entries(source).map(([name, skill]: [string, any]) => ({
						name,
						category: skill?.category,
						description: skill?.description,
						enabled: Boolean(skill?.enabled ?? skill?.active)
	...
	const toggle = async (skill: Skill) => {
		...
		try {
			await basculerCompetence(localStorage.token, skill.name, next);
		} catch (err) {
			skills = skills.map((s) => (s.name === skill.name ? { ...s, enabled: !next } : s));
...
<div class="w-full">
	...
	{:else if erreur}
		...
		>
			<div class="text-sm font-medium">{$i18n.t('Le moteur ne répond pas')}</div>
	...
	{:else}
		...
		<input
			...
			aria-label={$i18n.t('Rechercher une compétence')}
			bind:value={search}
		/>
