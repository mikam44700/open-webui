<script lang="ts">
	...
	const test = async () => {
		...
		try {
			...
			if (r?.ok)
				toast.success(
					$i18n.t('Connexion réussie') + (r.tools_count != null ? ` (${r.tools_count} outils)` : '')
				);
...
<div
	class="flex flex-col gap-2.5 p-4 rounded-2xl border border-gray-100 dark:border-gray-850 h-full card-lift hover:border-gray-200 dark:hover:border-gray-700 {connector.enabled
		? ''
		: 'opacity-60'}"
>
	...
	<div
		class="mt-auto flex items-center justify-end gap-2 pt-2 border-t border-gray-100 dark:border-gray-850"
	>
		<button
			type="button"
