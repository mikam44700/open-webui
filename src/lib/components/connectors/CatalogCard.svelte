<script lang="ts">
	...
	// Badge « MCP » affiché UNIQUEMENT dans « Recherche & web » (pour distinguer les
	// connecteurs MCP des fournisseurs natifs). Dans l'onglet MCP c'est redondant → masqué.
	...
	// Un MCP du registre non encore installable en 1 clic.
	...
	const waitInstall = async () => {
		for (let i = 0; i < 60; i++) {
			...
			const s = await getInstallStatus().catch(() => null);
			if (cancelled) return false;
		...
		try {
			...
			if (remoteConfig) {
				...
				if (remoteConfig.auth_type === 'oauth') {
					...
					keyValue = '';
					toast.success($i18n.t('Connecteur connecté.'));
					dispatch('changed');
				} else {
					toast.success($i18n.t('Connecteur connecté.'));
					dispatch('changed');
			...
			toast.error($i18n.t('Impossible de connecter ce service.'));
		} finally {
			working = false;
		}
	};
...
>
	...
	{#if actions.length > 0}
		...
		{:else}
			<div class="flex flex-col gap-1.5">
				...
				<ul class="flex flex-col gap-1 pl-0.5">
					{#each actions as action}
						<li class="flex items-start gap-1.5 text-[11px] text-gray-600 dark:text-gray-400">
							<span class="mt-1 size-1 flex-none rounded-full bg-gray-400 dark:bg-gray-600"></span>
				...
				<button
					type="button"
					class="self-start text-xs text-gray-400 transition hover:text-gray-600 dark:hover:text-gray-200"
					aria-expanded="true"
	...
	{:else if !entry.installed}
		<div class="mt-auto flex flex-col gap-2.5">
			<!-- Les champs nécessaires (clé/dossier…) sont révélés au premier clic. -->
