<script lang="ts">
	...
	// Hermes V2 ne livre aujourd'hui que quelques manifests. On les superpose au catalogue
	// historique complet de LunarIA : les métadonnées du moteur gagnent, mais le classement
	// V1 (catégorie/visibilité) reste la source de vérité de l'interface.
	$: engineById = new Map(entries.map((e) => [e.name, e]));
	$: historicalIds = new Set(LUNARIA_MCP_CATALOG.map((e) => e.name));
	$: historicalEntries = LUNARIA_MCP_CATALOG.map((fallback) => {
		const engine = engineById.get(fallback.name);
		return {
			...fallback,
			...engine,
			category: fallback.category,
			visibility: fallback.visibility,
			installed: engine?.installed ?? installedIds.has(fallback.name),
			// Un manifest Hermes réellement présent remplace l'état de repli. Les serveurs
			// HTTP officiels du catalogue historique gardent leur preset connectable.
			installable: engine ? engine.installable !== false : fallback.installable,
			preset: engine ? undefined : fallback.preset
		} as Entry;
	});
