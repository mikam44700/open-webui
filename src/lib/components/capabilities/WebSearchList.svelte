<script lang="ts">
	...
	// Cette page reste volontairement limitée aux quatre choix retenus par Michael.
	// Tous les autres fournisseurs restent accessibles depuis « Tout parcourir ».
	const MAIN_PROVIDER_SLUGS = new Set(['exa', 'brave', 'tavily']);
	// Déjà branchés = clé enregistrée ou compte connecté (état réel, jamais inventé). Remontés EN HAUT.
	$: connected = items.filter(
		(it) =>
			!!it.provider.slug &&
			MAIN_PROVIDER_SLUGS.has(it.provider.slug) &&
	...
	const FEATURED_SLUGS = ['exa', 'brave', 'tavily'];
	$: featured = (
