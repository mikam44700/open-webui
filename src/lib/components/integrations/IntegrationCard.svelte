<script lang="ts">
	...
	const CARD_TAGS_BY_ID: Record<string, string[]> = {
	...
	$: subservices =
		integration.subservices && integration.subservices.length > 0
			? integration.subservices
			: (CARD_TAGS_BY_ID[integration.id] ?? []);
...
>
	<div class="flex items-start gap-2.5">
		{#if logo}
			<div
				class="size-12 flex-none rounded-xl border border-gray-100 dark:border-gray-700 overflow-hidden flex items-center justify-center {logoFullBleed
					? ''
					: `${logoBg} p-0.5`}"
			>
				<img
	...
	<div class="mt-auto flex flex-col gap-2 pt-1">
		{#if isKeyLike && !isCentralOAuth && showField}
			<input
				...
				placeholder={isPath
					? $i18n.t('Chemin du dossier (ex. ~/Documents/Mon coffre)')
					: $i18n.t('Colle ta clé / ton token')}
				...
				{#each fr.actions as action}
					...
					<li class="flex items-start gap-2.5 text-xs text-gray-600 dark:text-gray-300">
						{#if svc && serviceLogoMap?.[svc]}
							...
							/>
						{:else}
							<span class="flex-none mt-1.5 size-1.5 rounded-full bg-gray-400 dark:bg-gray-600"
							></span>
						{/if}
						<span>
