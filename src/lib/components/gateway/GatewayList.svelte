<script lang="ts">
	...
	import googleChatLogo from '$lib/assets/messaging/google-chat.png';
	import microsoftTeamsLogo from '$lib/assets/messaging/microsoft-teams.png';
	...
	// LunarIA ne présente au client que les canaux retenus pour l'offre. Les autres
	// plateformes Hermes restent installées et configurables côté moteur : elles sont
	// uniquement masquées dans cette interface.
	const VISIBLE_PLATFORM_IDS = new Set([
		'telegram',
		'discord',
		'slack',
		'signal',
	...
	const ADVANCED_PLATFORM_IDS = new Set([
		'signal',
		'sms',
		'email',
		'google_chat',
	...
	$: filtered = platforms.filter(
		(p) => VISIBLE_PLATFORM_IDS.has(p.id) && ($expertMode || !ADVANCED_PLATFORM_IDS.has(p.id))
	);
