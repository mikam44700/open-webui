...
>
	...
	<header class="relative z-10 flex items-center justify-between px-5 py-4 md:px-8">
		<button class="flex items-center gap-3" on:click={leaveForNow}>
		...
		<div class="flex items-center gap-2">
			...
			<button
				class="rounded-full px-3 py-1.5 text-xs text-gray-500 hover:bg-black/5 dark:hover:bg-white/5"
				on:click={leaveForNow}>Reprendre plus tard</button
			>
		</div>
	</header>

	<main
		class="relative z-10 mx-auto flex min-h-[calc(100vh-72px)] w-full max-w-6xl flex-col px-5 pb-12 md:px-8"
	>
		{#if step === 'welcome'}
			<section class="m-auto grid w-full items-center gap-10 py-10 lg:grid-cols-[1.05fr_0.95fr]">
				<div>
					<div
						class="mb-5 inline-flex items-center gap-2 rounded-full border border-[#6b62f2]/25 bg-[#6b62f2]/8 px-3 py-1.5 text-xs font-medium text-[#5b52dd] dark:text-[#aaa4ff]"
					>
						<span class="size-1.5 rounded-full bg-[#6b62f2]"></span>
						Votre entreprise, comprise avant d’être automatisée
					</div>
					<h1 class="max-w-3xl text-4xl font-medium leading-[1.06] tracking-[-0.04em] md:text-6xl">
						Construisons le cerveau opérationnel de votre entreprise.
					</h1>
					<p class="mt-6 max-w-2xl text-base leading-7 text-gray-600 dark:text-gray-300 md:text-lg">
						LunarIA analyse votre activité, vous pose les bonnes questions et prépare les workflows
						réellement utiles à votre équipe.
					</p>
					<div class="mt-8 flex flex-wrap gap-3">
						<button
							class="rounded-full bg-[#6b62f2] px-6 py-3 text-sm font-medium text-white transition hover:bg-[#5a52dc]"
							on:click={begin}>Commencer l’entretien</button
						>
						{#if alreadyDone}
							<button
								class="rounded-full border border-gray-200 bg-white px-6 py-3 text-sm font-medium dark:border-gray-800 dark:bg-[#161616]"
								on:click={() => goto('/memoire')}>Voir ma mémoire</button
							>
						{/if}
					</div>
					<p class="mt-4 text-xs text-gray-500">
						Vous pourrez revenir en arrière et reprendre plus tard.
					</p>
				</div>

				<div
					class="rounded-[2rem] border border-black/6 bg-white/80 p-5 shadow-sm backdrop-blur dark:border-white/8 dark:bg-[#161616]/85"
				>
					<div class="mb-5 text-xs font-semibold uppercase tracking-[0.16em] text-gray-400">
						Ce que LunarIA va construire
					</div>
					<div class="space-y-3">
						{#each [['01', 'Comprendre', 'Votre activité, vos clients et votre marché'], ['02', 'Apprendre', 'Vos processus, priorités et règles internes'], ['03', 'Structurer', 'La Carte opérationnelle de votre entreprise'], ['04', 'Recommander', 'Un à trois workflows mesurables et encadrés']] as item}
							<div
								class="flex gap-4 rounded-2xl border border-black/5 bg-black/[0.015] p-4 dark:border-white/6 dark:bg-white/[0.025]"
							>
								<div
									class="flex size-9 shrink-0 items-center justify-center rounded-xl bg-[#6b62f2]/10 text-xs font-semibold text-[#6b62f2]"
								>
									{item[0]}
								</div>
								<div>
									<div class="text-sm font-medium">{item[1]}</div>
				...
				>
					...
					<div class="mt-6 flex flex-wrap items-center justify-between gap-3">
						...
						<div class="flex items-center gap-2">
							...
							>
								{questionIndex === questions.length - 1 ? 'Construire ma Carte' : 'Continuer'}
							</button>
						</div>
					</div>
				</div>
		...
		{:else if step === 'done'}
			<section class="m-auto w-full max-w-4xl py-10 text-center">
				<div
					class="mx-auto flex size-20 items-center justify-center rounded-[1.75rem] bg-emerald-500 text-white"
				>
					<svg viewBox="0 0 24 24" class="size-9" fill="none" stroke="currentColor" stroke-width="2"
						><path d="m5 12 4 4L19 6" /></svg
					>
				</div>
				<div class="mt-7 text-xs font-semibold uppercase tracking-[0.16em] text-emerald-600">
					Votre AgentOS a sa fondation
				</div>
				<h1 class="mt-3 text-3xl font-medium tracking-[-0.03em] md:text-5xl">
					{map.companyName || 'Votre entreprise'} a maintenant une mémoire opérationnelle.
				</h1>
				<p class="mx-auto mt-4 max-w-2xl text-sm leading-6 text-gray-500 dark:text-gray-400">
					{#if finishedWithWorkflow}
						Le workflow « {finishedWithWorkflow.title} » est préparé. Il ne sera pas exécuté avant son
						chantier dédié et votre validation.
					{:else}
						Votre Carte est enregistrée. Vous pourrez choisir le premier workflow plus tard.
					{/if}
				</p>
				...
				<div class="mx-auto mt-9 grid max-w-3xl gap-3 text-left sm:grid-cols-3">
					<a
						href="/memoire"
						class="rounded-2xl border border-black/6 bg-white p-5 transition hover:border-[#6b62f2]/40 dark:border-white/8 dark:bg-[#161616]"
					>
						<div class="text-sm font-medium">Voir ma Carte</div>
						<div class="mt-2 text-xs leading-5 text-gray-500">
							Retrouvez les faits, sources et règles dans Mémoire.
						</div>
					</a>
					<a
						...
						class="rounded-2xl border border-black/6 bg-white p-5 transition hover:border-[#6b62f2]/40 dark:border-white/8 dark:bg-[#161616]"
					>
						<div class="text-sm font-medium">Connecter mes outils</div>
						<div class="mt-2 text-xs leading-5 text-gray-500">
							{integrationNames.length
								? `${integrationNames.length} intégration(s) peuvent être utiles.`
								: 'Ajoutez les sources de vérité nécessaires.'}
						</div>
					</a>
					<button
						on:click={() => goto('/')}
						class="rounded-2xl border border-[#6b62f2]/30 bg-[#6b62f2]/8 p-5 text-left transition hover:bg-[#6b62f2]/12"
					>
						<div class="text-sm font-medium text-[#5b52dd] dark:text-[#aaa4ff]">
							Entrer dans LunarIA
						</div>
						<div class="mt-2 text-xs leading-5 text-gray-500 dark:text-gray-400">
							Votre prochaine action est prête.
						</div>
					</button>
				</div>
			</section>
