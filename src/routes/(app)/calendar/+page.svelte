<script lang="ts">
	// Page « Calendrier » d'Agent OS — pilotée par Hermes (feature 014, Google Agenda).
	// Remplace la surface native OpenWebUI (conservée dans l'historique + composants natifs
	// intacts) : Hermes est le seul maître du calendrier. Admin-only.
	import { onMount, getContext } from 'svelte';
	import { goto } from '$app/navigation';
	import { WEBUI_NAME, showSidebar, mobile, user } from '$lib/stores';

	import CalendarHermes from '$lib/components/calendar/CalendarHermes.svelte';
...
<svelte:head>
	<title>{$i18n.t('Calendrier')} • {$WEBUI_NAME}</title>
</svelte:head>
